...
template <typename T> static T *getDataPointer(Py::Object column);

MdfFileWrapper::MdfFileWrapper(Py::PythonClassInstance *self,
                               Py::Tuple &args,
                               Py::Dict &kwds)
    : Py::PythonClass<MdfFileWrapper>::PythonClass(self, args, kwds) {
  // Check the arguments. If only one is present, it should match the
  // IFileInterface. If two are present, the second should match the targeted
  // cache size.
  if (args.size() == 0) {
  ...
  try {
    if (args.size() > 1) {
      ...
      stream =
          std::make_shared<mdf::python::CallbackBuffer>(fileHandle, cacheSize);
    } else {
...
Py::Object MdfFileWrapper::GetDataFrame() {
  ...
  // NOTE: datetime64[ns] is represented as a floating point and not an integer
  // in numpy.
  ...
  // NOTE: The raw pointer access to databytes is not present, since this an
  ...
  for (auto const &record : iterator) {
    ...
    *columnEDL_ptr++ = record.EDL;
    ...
    // dataBytesArg = Py::TupleN(Py::Long(ctr), Py::Bytes(ptr,
    // record.DataBytes.size()));
    // columnDataBytes.callMemberFunction("__setitem__", dataBytesArg);
  }
  ...
  indexArgs["tz"] = utc;
  Py::Object index = pd.callMemberFunction(
      "DatetimeIndex", Py::TupleN(columnTimeStamp), indexArgs);
  ...
  Py::Object dataFrame =
      pd.callMemberFunction("DataFrame", Py::Tuple(), dataFrameArgs);

  // Merge the columns into the data frame.
  Py::Tuple insertArgs;
  int columnCounter = 0;

  insertArgs = Py::TupleN(Py::Long(columnCounter++), Py::String("BusChannel"),
                          columnBusChannel);
  dataFrame.callMemberFunction("insert", insertArgs);

  insertArgs =
      Py::TupleN(Py::Long(columnCounter++), Py::String("ID"), columnID);
  dataFrame.callMemberFunction("insert", insertArgs);

  insertArgs =
      Py::TupleN(Py::Long(columnCounter++), Py::String("IDE"), columnIDE);
  dataFrame.callMemberFunction("insert", insertArgs);

  insertArgs =
      Py::TupleN(Py::Long(columnCounter++), Py::String("DLC"), columnDLC);
  dataFrame.callMemberFunction("insert", insertArgs);

  insertArgs = Py::TupleN(Py::Long(columnCounter++), Py::String("DataLength"),
                          columnDataLength);
  dataFrame.callMemberFunction("insert", insertArgs);

  insertArgs =
      Py::TupleN(Py::Long(columnCounter++), Py::String("Dir"), columnDir);
  dataFrame.callMemberFunction("insert", insertArgs);

  insertArgs =
      Py::TupleN(Py::Long(columnCounter++), Py::String("EDL"), columnEDL);
  dataFrame.callMemberFunction("insert", insertArgs);

  insertArgs =
      Py::TupleN(Py::Long(columnCounter++), Py::String("BRS"), columnBRS);
  dataFrame.callMemberFunction("insert", insertArgs);

  insertArgs = Py::TupleN(Py::Long(columnCounter++), Py::String("DataBytes"),
                          columnDataBytes);
  dataFrame.callMemberFunction("insert", insertArgs);
...
void MdfFileWrapper::init_type() {
  ...
  behaviors().doc(
      "Read-only representation of a MDF file containing bus logging data");
  behaviors().supportGetattro();
  behaviors().supportSetattro();

  // Check if the requirements for reading out into pandas data frames are
  // present.
  ...
  BOOST_LOG_SEV(pythonLogger::get(), severity_level::trace)
      << "Checking for requirements for export to data frames";
  ...
  if (datetimeIsPresent == nullptr) {
    BOOST_LOG_SEV(pythonLogger::get(), severity_level::trace)
        << "datetime is not present";
    toDataFrameRequirementsPresent = false;
  }

  if (toDataFrameRequirementsPresent) {
    BOOST_LOG_SEV(pythonLogger::get(), severity_level::trace)
        << "Enabling data frame export";

    PYCXX_ADD_NOARGS_METHOD(get_data_frame, GetDataFrame,
                            "Get data frame over CAN records");
  }

  PYCXX_ADD_NOARGS_METHOD(
      get_first_measurement, GetFirstMeasurement,
      "Get the timestamp of the first measurement in the data");

  PYCXX_ADD_NOARGS_METHOD(get_can_iterator, GetCANIterator,
...
static Py::Object createNumpyArray(std::size_t size, std::string const &dtype) {
  ...
  return np.callMemberFunction("empty",
                               Py::TupleN(Py::Long(size), np.getAttr(dtype)));
}
...
template <typename T> static T *getDataPointer(Py::Object column) {
  ...
  Py::String columnID_arrayInterface_typestr =
      columnID_arrayInterface.getItem("typestr");
  if (columnID_arrayInterface_typestr.isNull() ||
      columnID_arrayInterface_typestr.isNone()) {
  ...
  if constexpr (sizeof(T) == 1) {
    if (byteCount != '1') {
      throw Py::RuntimeError(
          "numpy array does not use the expected number of bytes");
    }
  ...
  Py::Tuple columnID_arrayInterface_data =
      columnID_arrayInterface.getItem("data");
  if (columnID_arrayInterface_data.isNull() ||
      columnID_arrayInterface_data.isNone()) {
    throw Py::RuntimeError("Could not get numpy pointer part 2");
  }

  Py::Long ptrAddress =
      static_cast<Py::Long>(columnID_arrayInterface_data.getItem(0));
