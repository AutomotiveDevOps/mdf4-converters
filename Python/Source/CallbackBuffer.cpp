...
CallbackBuffer::CallbackBuffer(Py::Object obj, std::size_t cacheSize)
    : parent(obj), bufferOffset(0), cacheSize(cacheSize) {
  BOOST_LOG_SEV(pythonLogger::get(), severity_level::trace)
      << "Constructing CallbackBuffer from " << obj << " with a cache size of "
      << cacheSize;

  // Check for compliance with the expected interface.
  do {
    BOOST_LOG_SEV(pythonLogger::get(), severity_level::trace)
        << "Checking for compliance of required methods";
    if (!obj.hasAttr("read")) {
      BOOST_LOG_SEV(pythonLogger::get(), severity_level::fatal)
          << "Python object does not fulfill requirements for being used in a "
             "CallbackBuffer, does it implement IFileInterface?";
      throw std::runtime_error("Python object does not fulfill interface "
                               "requirements: Missing read method");
    }
    if (!obj.hasAttr("seek")) {
      BOOST_LOG_SEV(pythonLogger::get(), severity_level::fatal)
          << "Python object does not fulfill requirements for being used in a "
             "CallbackBuffer, does it implement IFileInterface?";
      throw std::runtime_error("Python object does not fulfill interface "
                               "requirements: Missing seek method");
    }

    auto const readMethod = obj.getAttr("read");
    auto const seekMethod = obj.getAttr("seek");

    auto const module = Py::Module("inspect");
    auto const readSpec =
        module.callMemberFunction("getfullargspec", Py::TupleN(readMethod));
    auto const seekSpec =
        module.callMemberFunction("getfullargspec", Py::TupleN(seekMethod));

    // Result should be a tuple, with the first value as a list.
    if (!readSpec.isSequence() || !seekSpec.isSequence()) {
      BOOST_LOG_SEV(pythonLogger::get(), severity_level::warning)
          << "Could not perform function introspection, getfullargspec";
      break;
    }
    ...
    if (readSequence.length() == 0 || seekSequence.length() == 0) {
      BOOST_LOG_SEV(pythonLogger::get(), severity_level::warning)
          << "Could not perform function introspection, sequence";
      break;
    }
    ...
    if (!readArgs.isList() || !seekArgs.isList()) {
      BOOST_LOG_SEV(pythonLogger::get(), severity_level::warning)
          << "Could not perform function introspection, list";
      break;
    }
    ...
    // Expect 2 arguments for read, and 2 for seek. Add an argument for each to
    // handle the self/this object.
    if (readArgsCount != 3 || seekArgsCount != 3) {
      BOOST_LOG_SEV(pythonLogger::get(), severity_level::warning)
          << "Could not perform function introspection, list count";
      break;
    }
...
CallbackBuffer::traits_type::int_type CallbackBuffer::underflow() {
  // Create bytes buffer.
  PyObject *pythonBuffer = PyMemoryView_FromMemory(
      reinterpret_cast<char *>(eback()), cacheSize, 0x200);
  Py::Object wrappedBuffer = Py::asObject(pythonBuffer);
  Py::Long bufferSizeObj(cacheSize);

  // Call in python.
  Py::Object result = parent.callMemberFunction(
      "read", Py::TupleN(wrappedBuffer, bufferSizeObj));
...
CallbackBuffer::pos_type
CallbackBuffer::seekoff(off_type offset,
...
CallbackBuffer::pos_type CallbackBuffer::seekpos(pos_type pos,
                                                 std::ios_base::openmode mode) {
...
CallbackBuffer::pos_type CallbackBuffer::seekBeginning(off_type offset) {
  ...
  if ((bufferOffset <= offset) &&
      (offset < (bufferOffset + currentAvailable))) {
    auto difference = offset - bufferOffset;
    setg(eback(), buffer + difference, egptr());
  } else {
    // Reset.
    Py::Object result = parent.callMemberFunction(
        "seek", Py::TupleN(Py::Long(offset), Py::Long(0)));
    Py::Long pos(result);
...
CallbackBuffer::pos_type CallbackBuffer::seekEnd(off_type offset) {
  Py::Object result = parent.callMemberFunction(
      "seek", Py::TupleN(Py::Long(offset), Py::Long(2)));
  Py::Long pos(result);
...
CallbackBuffer::pos_type CallbackBuffer::seekCurrent(off_type offset) {
  ...
  if ((bufferOffset <= newAbsolute) &&
      (newAbsolute < (bufferOffset + currentAvailable))) {
    auto difference = newAbsolute - bufferOffset;
    setg(eback(), buffer + difference, egptr());
  } else {
    // Reset.
    Py::Object result = parent.callMemberFunction(
        "seek", Py::TupleN(Py::Long(offset), Py::Long(1)));
    Py::Long pos(result);
