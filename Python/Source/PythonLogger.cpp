...
BOOST_LOG_GLOBAL_LOGGER_INIT(
    pythonLogger, boost::log::sources::severity_logger<severity_level>) {
...
bool setupLogging() {
  ...
  try {
    ...
    boost::shared_ptr<python_sink_t> pythonSink =
        boost::make_shared<python_sink_t>();
...
PythonLogger::PythonLogger() {
  ...
  auto pythonLoggerModuleRaw = PyImport_ImportModule("logging");

  if (pythonLoggerModuleRaw != nullptr) {
    auto pythonLoggerModule = Py::asObject(pythonLoggerModuleRaw);

    // Create a new logger for the extension and library.
    pythonLogger = pythonLoggerModule.callMemberFunction(
        "getLogger", Py::TupleN(Py::String("mdf_iter")));
