...
long long findMatchingLocation(std::shared_ptr<DGBlock> dgBlock,
                               std::vector<std::shared_ptr<CGBlock>> targets);

UnfinalizedFileInfo::UnfinalizedFileInfo(
    std::shared_ptr<HDBlock> hdBlock,
    std::shared_ptr<std::streambuf> dataSource)
    : hdBlock(std::move(hdBlock)), dataSource(std::move(dataSource)) {}
...
std::chrono::nanoseconds UnfinalizedFileInfo::firstMeasurement() const {
  ...
  // Extract records from the relevant DT blocks until a record from one of the
  // master channels is located.
  ...
  while (dgBlock) {
    ...
    while (cgBlock) {
      if (std::find(masterChannels.begin(), masterChannels.end(), cgBlock) !=
          masterChannels.end()) {
    ...
    do {
      ...
      dataSource->pubseekoff(masterChannel->getByteOffset(),
                             std::ios_base::cur);

      // Determine how many bytes to read out.
      std::size_t bitsToRead =
          masterChannel->getBitCount() + masterChannel->getBitOffset();
      ...
      // TODO: If multiple DG are present, this needs to take those into
      // account. Since this is not the
...
long long findMatchingLocation(std::shared_ptr<DGBlock> dgBlock,
  ...
  std::shared_ptr<DTBlockMultipleRecordIDs> dtBlock =
...
std::shared_ptr<CNBlock>
getMasterTimeChannel(std::shared_ptr<CGBlock> cgBlock) {
  ...
  while (cnBlock) {
    ...
    // NOTE: No need to check composition blocks, as they cannot contain the
    // master channel.
    cnBlock = cnBlock->getNextCNBlock();
  }

  // If a result was located, cnBlock is a valid pointer to the master channel.
  // Else, it has no reference.
  return cnBlock;
}
