...
bool MdfFileImplementation::finalize_setLengthOfLastDTBlock() {
  ...
  if (highestDataBlock) {
    ...
    if (iter != std::end(blockStorage->getBlockMap().left)) {
      // TODO: Bound by another block. Use the address of this block to
      // calculate the size. Simply throw for now.
      throw std::runtime_error("Data block is not bounded by end of file but "
                               "by another block. Not supported in library.");
    } else {
      ...
      dtHeader.blockSize =
          static_cast<uint64_t>(end) - highestDataBlockLocation;
...
bool MdfFileImplementation::finalize_updateCycleCountersInCGCABlocks() {
  // Index the records, to get the correct number for each channel group. Loop
  // over each DG block.
  for (auto &it : blockStorage->getBlockMap().left) {
    if (it.second->getHeader().blockType == MdfBlockType_DG) {
      std::shared_ptr<DGBlock> dgBlock =
          std::dynamic_pointer_cast<DGBlock>(it.second);
      ...
      if ((cgBlock) && (cgBlock->getNextCGBlock())) {
        ...
        auto dtBlock = std::dynamic_pointer_cast<DTBlockMultipleRecordIDs>(
        ...
        auto dtBlock = std::dynamic_pointer_cast<DTBlockSingleDiscontinuous>(
            dgBlock->getDataBlock());
...
bool MdfFileImplementation::finalize_updateByteCountersInVLSDCGBlocks() {
  ...
  for (auto &it : blockStorage->getBlockMap().left) {
    if (it.second->getHeader().blockType == MdfBlockType_DG) {
      ...
      while (cgBlock) {
        if (cgBlock->isVLSD()) {
          // CGVLSD blocks must share a DG block, so this is unsorted data and
          // thus multiple records per ID.
          auto dtBlock = std::dynamic_pointer_cast<DTBlockMultipleRecordIDs>(
              dgBlock->getDataBlock());
...
FileInfo MdfFileImplementation::getFileInfo() const { return fileInfo; }
  ...
  do {
    ...
    // Load data from the HD block.
    {
      ...
      if (mdBlock) {
        ...
        loadStatus = getSharedInfo(fileInfo, metaData);
      }
    ...
    // Locate CAN data.
    {
      std::shared_ptr<DGBlock> CANDGBlock = findBUSBlock(SIBlockBusType::CAN);

      if (CANDGBlock) {
        std::shared_ptr<CGBlock> CANCGBlock = CANDGBlock->getFirstCGBlock();

        // Extract cycle count.
        fileInfo.CANMessages = CANCGBlock->getCycleCount();

        // Get xml data from the acquisition source.
        bool loadStatus = false;
        std::shared_ptr<SIBlock> siBlock = CANCGBlock->getSIBlock();
        if (siBlock) {
          ...
          if (mdBlock) {
            // Extract common metadata.
            std::string metaData(mdBlock->getMetaData());
            loadStatus = getCANInfo(fileInfo, metaData);
          }
        }
    ...
    // Locate LIN data.
    {
      std::shared_ptr<DGBlock> LINDGBlock = findBUSBlock(SIBlockBusType::LIN);
...
/*
bool MdfFileImplementation::load(std::string fileName) {
    bool result = false;
    ...
    do {
        ...
        blockStorage = std::make_unique<BlockStorage>(mmap);

        // Load the ID block.
        idBlock = std::make_unique<IDBlock>();
        auto dataPtr = reinterpret_cast<uint8_t const *>(mmap.data());
        result = idBlock->load(dataPtr);
    ...
    // If an error occurred, ensure the file is un-mapped.
    if (!result) {
        if (mmap.is_open()) {
            mmap.unmap();
        }
...
bool MdfFileImplementation::sort() {
  // Sorting requires each DG block to have only a single CG block. Thus, loop
  // over all DG blocks, and for each CG block after the first, move it to a new
  // DG block. For VLSD CG blocks, instead of moving them to a new DG block,
  // convert them into a SD block, and unlink them from the CG block chain (But
  // ensure the link in the CN block referencing it stays intact).
  bool result = false;
  ...
  /*
  // Post sorting, each DT block can only contain a single record ID. If the
  file was sorted this run, they are
  // replaced with in-memory mapped DT blocks. If it was sorted prior to this
  run, replace the DT blocks. std::shared_ptr<DGBlock> dgBlock =
  getHDBlock()->getFirstDGBlock();

  while(dgBlock) {
    // Loop over all linked DT blocks and replace them if not already single ID
  blocks.

    // Check if it is of the right type.
    std::shared_ptr<DTBlock> dtBlock =
  std::dynamic_pointer_cast<DTBlock>(dgBlock->getDataBlock());

    if(dtBlock) {
      // Ensure it is a single ID block.
      std::shared_ptr<DTBlockRaw> dtBlockRaw =
  std::dynamic_pointer_cast<DTBlockRaw>(dtBlock);
      ...
      if(!dtBlockRaw) {
        ...
        replacementDTBlock =
  std::make_shared<DTBlockSingleContinuous>(*dtBlockRaw, recordSize);
...
bool MdfFileImplementation::sort_VLSDCGtoSD() {
  // For each DG block, find all CG blocks. For each CG block which is a VLSD
  // block, move it to a SD block instead.
  std::shared_ptr<HDBlock> hdBlock =
      std::dynamic_pointer_cast<HDBlock>(blockStorage->getBlockAt(64));
  std::shared_ptr<DGBlock> dgBlock = hdBlock->getFirstDGBlock();

  // TODO: This construction is a bit convoluted, think of possible
  // simplifications.
  while (dgBlock) {
    ...
    for (auto const &block : cgBlocks) {
      ...
      if (nextCGBlock && nextCGBlock->isVLSD()) {
        ...
        std::shared_ptr<DTBlockMultipleRecordIDs> dataBlock =
            std::dynamic_pointer_cast<DTBlockMultipleRecordIDs>(
                dgBlock->getDataBlock());
        uint64_t totalSize =
            dataBlock->getTotalSize(nextCGBlock->getRecordID());
        auto records =
            dataBlock->getRecordIndicesAbsolute(nextCGBlock->getRecordID());

        totalSize += 4 * nextCGBlock->getCycleCount();

        // Create a SD block to store the data in.
        std::shared_ptr<SDBlockDiscontinuous> sdBlock =
            std::make_shared<SDBlockDiscontinuous>(records, stream);
        ...
        recordsIDs.erase(std::remove(std::begin(recordsIDs),
                                     std::end(recordsIDs),
                                     nextCGBlock->getRecordID()),
                         std::end(recordsIDs));
        ...
        if (recordsIDs.size() > 1) {
          replacementDTBlock = std::make_shared<DTBlockMultipleRecordIDs>(
              *dataBlock, recordsIDs);
        } else if (recordsIDs.size() == 1) {
          replacementDTBlock = std::make_shared<DTBlockSingleDiscontinuous>(
              *dataBlock, recordsIDs[0]);
        }
...
bool MdfFileImplementation::sort_CGtoDG() {
  ...
  while (dgBlock) {
    ...
    if (followingCGBlock) {
      ...
      auto currentDataBlock =
          std::dynamic_pointer_cast<DTBlockMultipleRecordIDs>(
              dgBlock->getDataBlock());
      std::vector<uint64_t> recordsIDs =
          currentDataBlock->getRegisteredRecordIDs();

      recordsIDs.erase(std::remove(std::begin(recordsIDs), std::end(recordsIDs),
                                   cgBlock->getRecordID()),
                       std::end(recordsIDs));

      auto replacementDTBlockForOldDGBlock =
          std::make_shared<DTBlockSingleDiscontinuous>(*currentDataBlock,
                                                       cgBlock->getRecordID());

      // Replace the old DT block with one without the removed ID.
      std::shared_ptr<DTBlock> replacementDTBlockForNewDGBlock;
      if (recordsIDs.size() > 1) {
        replacementDTBlockForNewDGBlock =
            std::make_shared<DTBlockMultipleRecordIDs>(*currentDataBlock,
                                                       recordsIDs);
      } else if (recordsIDs.size() == 1) {
        replacementDTBlockForNewDGBlock =
            std::make_shared<DTBlockSingleDiscontinuous>(*currentDataBlock,
...
bool MdfFileImplementation::save(std::string fileName) {
  ...
  for (auto const &iter: blockStorage->getPackedBlockMap()) {
      ...
      if (currentBlockSize % sizeof(uint64_t) != 0) {
          currentBlockSize += sizeof(uint64_t) - currentBlockSize %
  sizeof(uint64_t);
      }
  ...
  // Save all the blocks.
  for (auto &block: blocks) {
      uint8_t *dataPtr = reinterpret_cast<uint8_t *>(outputFile.data()) +
  block.first; block.second->save(dataPtr);
  }
*/
  return result;
}
...
std::shared_ptr<DGBlock>
MdfFileImplementation::findBUSBlock(SIBlockBusType busType) const {
  ...
  while (dgBlock) {
    ...
    do {
      ...
      if ((cgBlock->getFlags() & CGBlockFlags::BUSEvents) !=
          CGBlockFlags::BUSEvents) {
        break;
      }
      ...
      if (!siBlock) {
        // No valid source information block, can not determine if this is the
        // correct bus type.
        break;
      }
...
RecordIterator<CANRecord const> MdfFileImplementation::getCANIterator() {
  ...
  if (dgBlock) {
    ...
    std::unique_ptr<IIterator<CANRecord const>> iter =
        std::make_unique<GenericIterator<CANRecord>>(dgBlock, stream,
                                                     fileInfo.Time);
...
RecordIterator<LINRecord const> MdfFileImplementation::getLINIterator() {
  ...
  if (dgBlock) {
    // Create a new iterator from this.
    std::unique_ptr<IIterator<LINRecord const>> iter =
        std::make_unique<GenericIterator<LINRecord>>(dgBlock, stream,
                                                     fileInfo.Time);
