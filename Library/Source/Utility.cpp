...
std::vector<std::shared_ptr<CNBlock>>
getAllCNBlocks(std::shared_ptr<CGBlock> parent) {
...
std::vector<std::shared_ptr<CNBlock>>
getAllCNBlocks(std::shared_ptr<CNBlock> parent) {
  std::vector<std::shared_ptr<CNBlock>> result;
  ...
  // First walk down composition channels.
  {
    ...
    std::vector<std::shared_ptr<CNBlock>> compositionResult =
        getAllCNBlocks(composition);

    // Merge the results.
    result.insert(std::end(result), std::begin(compositionResult),
                  std::end(compositionResult));
  }

  // Next, walk down the channels.
  {
    std::shared_ptr<CNBlock> nextChannel = parent->getNextCNBlock();
    std::vector<std::shared_ptr<CNBlock>> nextChannelResult =
        getAllCNBlocks(nextChannel);

    // Merge the results.
    result.insert(std::end(result), std::begin(nextChannelResult),
                  std::end(nextChannelResult));
  }
