...
bool MdfBlockSortingOrder::operator()(
    const std::shared_ptr<MdfBlock> &lhs,
    const std::shared_ptr<MdfBlock> &rhs) const {
...
BlockStorage::BlockStorage(std::shared_ptr<std::streambuf> stream)
    : stream(stream) {}
...
std::shared_ptr<MdfBlock> BlockStorage::getBlockAt(uint64_t address) {
  ...
  if (iter == std::end(blockMap.left)) {
    ...
    for (uint64_t linkNumber = 0; linkNumber < header.linkCount; linkNumber++) {
      ...
      bytesRead = stream->sgetn(reinterpret_cast<char *>(linkLocation.data()),
                                sizeof(linkLocation));
      ...
      // Recursive loading. Since this seeks to a new position, store the
      // current location.
    ...
    if (result) {
      ...
      for (std::size_t i = 0; i < links.size(); ++i) {
        if (links[i] != newLinks[i]) {
          // A block has been replaced. Find the old block in the list and
          // replace it.
...
const boost::bimap<uint64_t, std::shared_ptr<MdfBlock>> &
BlockStorage::getBlockMap() const {
  return blockMap;
}

std::map<uint64_t, std::shared_ptr<MdfBlock>>
BlockStorage::getPackedBlockMap() {
  ...
  // Create a map, starting with the HD block, putting each block at the next
  // available slot.
...
BlockStorage::getLinkedBlocks(std::shared_ptr<MdfBlock> startBlock) {
  std::set<std::shared_ptr<MdfBlock>, MdfBlockSortingOrder> result =
      std::set<std::shared_ptr<MdfBlock>, MdfBlockSortingOrder>();
