...
DTBlockSingleContinuous::DTBlockSingleContinuous(const mdf::DTBlock &parent,
                                                 std::size_t recordSize)
    : recordSize(recordSize), DTBlock(parent) {
  //
}

uint64_t DTBlockSingleContinuous::operator[](std::size_t index) {
  // Since this is a continuous block, the n'th record is simple an offset into
  // the data block.
