...
DTBlockMultipleRecordIDs::DTBlockMultipleRecordIDs(DTBlock const &parent)
    : DTBlock(parent) {}

DTBlockMultipleRecordIDs::DTBlockMultipleRecordIDs(
    DTBlockMultipleRecordIDs const &parent, std::vector<uint64_t> IDs)
    : DTBlock(parent) {
  // Reset header.
  ...
  for (auto &recordID : IDs) {
    recordIndices.insert(
        std::make_pair(recordID, parent.recordIndices.at(recordID)));
    recordSizes.insert(
        std::make_pair(recordID, parent.recordSizes.at(recordID)));
    recordSizeMap.insert(
        std::make_pair(recordID, parent.recordSizeMap.at(recordID)));
...
DTBlockMultipleRecordIDs::getRecordCycleCounts() const {
...
std::vector<uint64_t>
DTBlockMultipleRecordIDs::getRecordIndicesAbsolute(uint64_t recordID) const {
...
long long
DTBlockMultipleRecordIDs::findFirstMatching(std::vector<uint64_t> targetIDs) {
  long long result = -1;

  std::size_t currentLocation =
      stream->pubseekoff(rawFileLocation + 24, std::ios_base::beg);
  ...
  while (currentLocation < endLocation) {
    ...
    if (std::find(targetIDs.begin(), targetIDs.end(), recordID) !=
        targetIDs.end()) {
    ...
    if (recordSize < 0) {
      ...
      stream->sgetn(reinterpret_cast<char *>(&VLSDrecordSize),
                    sizeof(VLSDrecordSize));
...
void DTBlockMultipleRecordIDs::index() {
  ...
  std::streampos const blockDataEnd =
      getFileLocation() + header.blockSize - sizeof(header);
  ...
  for(auto const& recordInformation: recordIndices) {
      ...
      if(recordSize < 0) {
          for(auto const& sourceAddress: recordInformation.second) {
              ...
              std::copy(input->recordData, input->recordData +
  input->recordSize.value(), output->recordData); dataPtr += sizeof(SDRecord) +
  input->recordSize.value();
          }
      } else {
          for(auto const& sourceAddress: recordInformation.second) {
...
void DTBlockMultipleRecordIDs::setRecordInformation(
    std::map<uint64_t, int64_t> recordSizes, uint64_t recordLength) {
  this->recordSizeMap = std::move(recordSizes);
  this->recordLength = recordLength;
}

uint64_t DTBlockMultipleRecordIDs::operator[](std::size_t index) { return 0; }
