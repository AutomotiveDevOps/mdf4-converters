...
SDBlockDiscontinuous::SDBlockDiscontinuous(
    std::vector<uint64_t> const &dataPtr,
    std::shared_ptr<std::streambuf> stream)
    : dataRecords(dataPtr), stream(stream) {
  // The data pointer has the indices of the start of each record. We want a map
  // with the offset as the key and the record as the value. Assumes data is
  // sorted.
  ...
  for (auto entry : dataRecords | boost::adaptors::indexed(0)) {
    ...
    stream->sgetn(reinterpret_cast<char *>(vlsdLength.data()),
                  sizeof(vlsdLength));
...
bool SDBlockDiscontinuous::saveBlockData(uint8_t *dataPtr) {
  for (auto const &record : dataRecords) {
    ...
    std::copy(input->recordData, input->recordData + input->recordSize.value(),
              output->recordData);
