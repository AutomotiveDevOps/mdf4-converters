...
uint8_t CNBlock::getBitOffset() const { return bitOffset; }

uint32_t CNBlock::getByteOffset() const { return byteOffset; }

uint32_t CNBlock::getBitCount() const { return bitCount; }

CNType CNBlock::getChannelType() const { return CNType(type); }

CNDataType CNBlock::getDataType() const { return CNDataType(dataType); }

CNSyncType CNBlock::getSyncType() const { return CNSyncType(syncType); }
...
std::shared_ptr<MdfBlock> CNBlock::getNameBlock() const { return links[2]; }

std::shared_ptr<MdfBlock> CNBlock::getSourceBlock() const { return links[3]; }

std::shared_ptr<MdfBlock> CNBlock::getDataBlock() const { return links[5]; }
...
bool CNBlock::load(std::shared_ptr<std::streambuf> stream) {
  ...
  std::streamsize bytesRead =
      stream->sgetn(reinterpret_cast<char *>(&rawData), sizeof(rawData));
