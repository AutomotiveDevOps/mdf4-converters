...
struct DTBlockMultipleRecordIDs : DTBlock {
  ...
  DTBlockMultipleRecordIDs(DTBlockMultipleRecordIDs const &parent,
                           std::vector<uint64_t> IDs);

  void setRecordInformation(std::map<uint64_t, int64_t> recordSizes,
  ...
  getRecordIndicesAbsolute(uint64_t recordID) const;
...
protected:
  ...
  /**
   * A lookup table containing each record offset in this block. The record ID
   * is the key, while a vector of offsets is the value.
   */
  ...
  /**
   * Map over records IDs (key) and sizes. For VLSD records, this size is set to
   * -1.
   */
