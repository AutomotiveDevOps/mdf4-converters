...
void MdfBlock::setHeader(MdfHeader header_) { this->header = header_; }

std::shared_ptr<MdfBlock>
createBlock(MdfHeader header,
...
const MdfHeader &MdfBlock::getHeader() const { return header; }

const std::vector<std::shared_ptr<MdfBlock>> &MdfBlock::getLinks() const {
  return links;
}

uint64_t MdfBlock::getFileLocation() const { return fileLocation; }
...
bool MdfBlock::save(uint8_t *dataPtr) {
  ...
  for (auto &link : links) {
    auto linkAddress =
        reinterpret_cast<boost::endian::little_uint64_buf_t *>(dataPtr);
