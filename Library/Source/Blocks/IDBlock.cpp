...
constexpr uint8_t const finalizedIdentifierStr[8] = {'M', 'D', 'F', ' ',
                                                     ' ', ' ', ' ', ' '};
constexpr uint8_t const unfinalizedIdentifierStr[8] = {'U', 'n', 'F', 'i',
                                                       'n', 'M', 'F', ' '};
...
bool IDBlock::load(std::shared_ptr<std::streambuf> stream) {
  ...
  do {
    ...
    IDBlockData const *const data =
        reinterpret_cast<IDBlockData const *>(buffer);

    // Determine if this is a MDF4 file at all.
    if (!std::equal(std::cbegin(data->identifierStr),
                    std::cend(data->identifierStr),
                    std::cbegin(finalizedIdentifierStr)) &&
        !std::equal(std::cbegin(data->identifierStr),
                    std::cend(data->identifierStr),
                    std::cbegin(unfinalizedIdentifierStr))) {
      break;
    }

    // Read out data.
    creationTool = std::string(reinterpret_cast<char const *>(data->toolStr),
                               sizeof(data->toolStr));
    versionNum = data->versionNum.value();
    versionStr = std::string(reinterpret_cast<char const *>(data->versionStr),
                             sizeof(data->versionStr));
...
bool IDBlock::save(uint8_t *dataPtr) {
  ...
  if ((finalizationFlags == FinalizationFlags_None) &&
      (customFinalizationFlags == 0)) {
    // Write as finalized.
    std::copy(std::cbegin(finalizedIdentifierStr),
              std::cend(finalizedIdentifierStr), ptr->identifierStr);
  } else {
    // Write as unfinalized.
    std::copy(std::cbegin(unfinalizedIdentifierStr),
              std::cend(unfinalizedIdentifierStr), ptr->identifierStr);
