...
constexpr MdfHeader DGBlockHeader = {
    .blockType = MdfBlockType_DG, .blockSize = 64, .linkCount = 4};
...
std::shared_ptr<MdfBlock> DGBlock::getDataBlock() const { return links[2]; }
...
bool DGBlock::load(std::shared_ptr<std::streambuf> stream) {
  ...
  while (cgBlock) {
    recordSizes.insert(
        std::make_pair(cgBlock->getRecordID(), cgBlock->getRecordSize()));
  ...
  // Create the correct type of DT block from the loaded DT block, depending on
  // the IDs.
  if (recordSizes.size() > 1) {
    // Multiple IDs. Send the information to the DT block and create an index.
    auto currentDTBlock =
        std::make_shared<DTBlockMultipleRecordIDs>(*originalDTBlock);
    ...
    auto currentDTBlock = std::make_shared<DTBlockSingleContinuous>(
        *originalDTBlock, cgBlock->getRecordSize());
...
void DGBlock::setRecordSize(uint8_t recordSize) { recordIDSize = recordSize; }

uint8_t DGBlock::getRecordSize() const { return recordIDSize; }
