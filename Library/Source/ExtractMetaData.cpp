...
bool getSharedInfo(FileInfo &fileInfo, std::string xml) {
  ...
  XMLElement const *cpElement =
      rootElement->FirstChildElement("common_properties");
  ...
  // There are two trees here with information: "Device Information" and "File
  // Information".
  for (XMLElement const *element = cpElement->FirstChildElement();
    ...
    if (strncmp(attributeName, "Device Information", strlen(attributeName)) ==
        0) {
      result &= parseDeviceInformation(fileInfo, element);
    } else if (strncmp(attributeName, "File Information",
                       strlen(attributeName)) == 0) {
...
bool getLINInfo(FileInfo &fileInfo, std::string xml) {
  ...
  for (XMLElement const *element = treeElement->FirstChildElement();
    ...
    } else if (strncmp(attributeName, "LIN2 Bit-rate", strlen(attributeName)) ==
               0) {
      uint32_t bitRate;
...
bool getCANInfo(FileInfo &fileInfo, std::string xml) {
  ...
  for (XMLElement const *element = treeElement->FirstChildElement();
    ...
    } else if (strncmp(attributeName, "CAN2 Bit-rate", strlen(attributeName)) ==
               0) {
      uint32_t bitRate;
...
bool parseDeviceInformation(FileInfo &fileInfo, XMLElement const *treeRoot) {
  // Loop over all the attributes, and map their properties to the file
  // information.
  for (XMLElement const *element = treeRoot->FirstChildElement();
    ...
    if (strncmp(attributeName, "firmware version", strlen(attributeName)) ==
        0) {
      if (elementValue == nullptr) {
        return false;
      }
      ...
      Version fwVersion;
      unsigned int patch;
      auto scanRes = sscanf(elementValue, "%2u.%2u.%2u", &fwVersion.major,
                            &fwVersion.minor, &patch);
    ...
    } else if (strncmp(attributeName, "hardware version",
                       strlen(attributeName)) == 0) {
      if (elementValue == nullptr) {
        return false;
      }
      ...
      Version hwVersion;
      auto scanRes =
          sscanf(elementValue, "%2u.%2u", &hwVersion.major, &hwVersion.minor);
    ...
    } else if (strncmp(attributeName, "serial number", strlen(attributeName)) ==
               0) {
      fileInfo.LoggerID = strtoul(elementValue, nullptr, 16);
    } else if (strncmp(attributeName, "device type", strlen(attributeName)) ==
               0) {
