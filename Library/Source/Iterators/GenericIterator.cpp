...
GenericIterator<RecordType>::GenericIterator(
    ...
    std::size_t start)
    : start(start), stream(stream), current(0) {
  ...
  if (!cgBlock) {
    ...
    throw std::runtime_error(
        "More than one CG block in DG block, not a sorted DG block");
  }
  ...
  for (auto const &cnBlock : cnBlocks) {
    ...
    auto nameBlock =
        std::dynamic_pointer_cast<TXBlock>(cnBlock->getNameBlock());
    ...
    if (cnBlock->getDataBlock()) {
      auto dataBlock =
          std::dynamic_pointer_cast<SDBlock>(cnBlock->getDataBlock());
    ...
    MappingInformationEntry m = {
        .byteOffset = static_cast<uint8_t>(cnBlock->getByteOffset()),
        .bitOffset = cnBlock->getBitOffset(),
        .bitLength = static_cast<uint8_t>(cnBlock->getBitCount()),
        .dataType = cnBlock->getDataType(),
        .dataFunc = f,
        .fieldName = std::string(name)};

    // If this is the master channel, it should be time.
    if ((cnBlock->getChannelType() == CNType::MasterChannel) &&
        (cnBlock->getSyncType() == CNSyncType::Time)) {
    ...
    std::size_t const currentBitIndex =
        m.byteOffset * 8 + m.bitOffset + m.bitLength;
  ...
  // Correlate mapping information with the record fields. Using ideas from
  // https://stackoverflow.com/a/39613483.
  mapping.clear();
  ...
  for (auto const &entry : mappingInformation | boost::adaptors::indexed(0)) {
    ...
    if (dotPos != std::string_view::npos) {
      // Get a handle to the remaining as the name of the channel for matching
      // against the fields in the RecordType.
      mapName = mapName.substr(dotPos + 1);
    }

    // Lambda function specific for this iteration, to compare the field name
    // and the entry name.
    auto f = [&](auto index) {
      // Attempt to match the member name by lowercase comparison. Need to store
      // the string here locally.
      std::string memberName =
          boost::fusion::extension::struct_member_name<RecordType,
                                                       index>::call();
    ...
    boost::fusion::for_each(
        boost::mpl::range_c<
            unsigned, 0, boost::fusion::result_of::size<RecordType>::value>(),
        f);
  ...
  // NOTE: May not need this, and can return to lambda functions, as long as
  // they are copied in the copy constructor.
...
template <typename RecordType>
GenericIterator<RecordType>::GenericIterator(
    const GenericIterator<RecordType> &other)
    : start(other.start), stop(other.stop), current(other.current),
      record(other.record), mappingInformation(other.mappingInformation),
      mapping(other.mapping), mappingEntries(other.mappingEntries),
      bitData(other.bitData), functorStorage(other.functorStorage),
      stream(other.stream) {
  func = functorStorage[0];
}

template <typename RecordType> void GenericIterator<RecordType>::increment() {
  if (current < stop) {
    current += 1;
    if (current < stop) {
      mapDataToRecord();
...
template <typename RecordType>
std::unique_ptr<IIterator<RecordType const>>
GenericIterator<RecordType>::clone() const {
  return std::make_unique<GenericIterator<RecordType>>(*this);
}

template <typename RecordType>
std::unique_ptr<IIterator<RecordType const>>
GenericIterator<RecordType>::begin() const {
...
template <typename RecordType>
std::unique_ptr<IIterator<RecordType const>>
GenericIterator<RecordType>::end() const {
  auto result = std::make_unique<GenericIterator<RecordType>>(*this);
...
void GenericIterator<RecordType>::mapDataToRecord() {
  ...
  // Use special assign functions from
  // https://www.meetingcpp.com/blog/items/Accessing-a-fusion-sequence-with-a-run-time-defined-index.html.
  ...
  auto f = [&](auto index) {
    ...
    if (iter != mapping.end()) {
      ...
      if (!entry.dataFunc) {
        ...
        assign3(boost::fusion::at_c<index>(record), *stream.get(), dataOffset);
      }

      // If a conversion function is specified, run it afterwards.
      if (entry.conversionFunc) {
        entry.conversionFunc(
            static_cast<void *>(&boost::fusion::at_c<index>(record)));
      }
    }
  };
  ...
  boost::fusion::for_each(
      boost::mpl::range_c<unsigned, 0,
                          boost::fusion::result_of::size<RecordType>::value>(),
      f);

  // Truncate data in the case of LIN records.
  if constexpr (std::is_same_v<LINRecord, RecordType>) {
    auto &r = reinterpret_cast<LINRecord &>(record);
...
template <typename RecordType>
bool GenericIterator<RecordType>::equals(
    IIterator<RecordType const> const &other) const {
...
template struct GenericIterator<CANRecord>;

template struct GenericIterator<LINRecord>;
