...
template <typename> struct is_static_vector2 : public std::false_type {};

template <typename T, std::size_t A>
struct is_static_vector2<boost::container::static_vector<T, A>>
    : public std::true_type {};

template <typename> struct static_vector_length {
...
void assign2(ToType &to, FromType const &from) {
  ...
  if constexpr (is_static_vector2<ToType>::value) {
    ...
    if (std::is_pointer_v<FromType>) {
      ...
      // Special case of VLSD data. Read the first 4 bytes to determine data
      // length.
      ...
      if (dataLength > ToType::static_capacity) {
        throw std::runtime_error(
            "Attempting to copy more bytes than the container can handle");
      }
    } else {
      basePtr = reinterpret_cast<uint8_t const *>(&from);
    ...
    // All other cases. May be needed at compile time, but are not allowed at
    // run time.
...
void assign3(boost::container::static_vector<ToType, A> &to,
