...
inline void swap(unique_impl_ptr<T, D> &l,
                 unique_impl_ptr<T, D> &r) SPIMPL_NOEXCEPT {
  l.swap(r);
}
...
template <class T, class D>
inline bool operator==(const unique_impl_ptr<T, D> &p,
...
inline bool operator==(std::nullptr_t,
                       const unique_impl_ptr<T, D> &p) SPIMPL_NOEXCEPT {
  return !p;
}
...
public:
  ...
  SPIMPL_CONSTEXPR impl_ptr() SPIMPL_NOEXCEPT
      : base_type(nullptr, deleter_type{}),
        copier_(copier_type{}) {}
  ...
  impl_ptr(
      ...
      typename std::enable_if<std::is_convertible<D, deleter_type>::value &&
                              ...
                              typename base_type::dummy_t_>::type =
          typename base_type::dummy_t_()) SPIMPL_NOEXCEPT
      : base_type(std::move(p), std::forward<D>(d)),
...
#else
  impl_ptr(impl_ptr &&r) SPIMPL_NOEXCEPT : base_type(std::move(r.ptr_)),
  ...
  impl_ptr clone() const {
    return impl_ptr(base_type::ptr_ ? copier_(base_type::ptr_.get()) : nullptr,
                    base_type::ptr_.get_deleter(), copier_);
  }
...
namespace std {
template <class T, class D> struct hash<spimpl::unique_impl_ptr<T, D>> {
