...
bool decompressFile(std::string const &inputFile,
  ...
  auto bufferPtr =
      reinterpret_cast<unsigned char const *>(bufferCompressionHeader);

  uint32_t lookaheadSize = boost::endian::load_big_u32(bufferPtr);
  uint32_t windowSize =
      boost::endian::load_big_u32(bufferPtr + sizeof(lookaheadSize));
  ...
  heatshrink_decoder *decoder =
      heatshrink_decoder_alloc(1024, windowSizeLog, lookaheadSize);
  ...
  while (!inputStream.eof() && !inputStream.fail()) {
    ...
    do {
      sinkResult = heatshrink_decoder_sink(decoder, dataBufferPtr, bytesRead,
                                           &bytesSunk);

      size_t bytesPolled;
      do {
        pollResult = heatshrink_decoder_poll(
            decoder, dataBufferPtr, dataBuffer.capacity(), &bytesPolled);
  ...
  while (finishResult == HSDR_FINISH_MORE) {
    size_t bytesPolled;
    heatshrink_decoder_poll(decoder, dataBufferPtr, dataBuffer.capacity(),
                            &bytesPolled);
