...
constexpr char magicHeader[12] = {'G', 'e', 'n', 'e', 'r', 'i',
                                  'c', ' ', 'F', 'i', 'l', 'e'};
...
bool isEncryptedFile(std::string const &fileName) {
  // Open the file and compare the structure against the expected structure, and
  // the supported capabilities by this library.
  ...
  // Ensure it is compressed, and that the compression is supported by this
  // library.
...
uint32_t getDeviceIdFromFile(std::string const &fileName) {
  ...
  result = boost::endian::load_big_u32(
      reinterpret_cast<uint8_t const *>(buffer) + 16);
...
bool decryptFile(std::string const &inputFile,
  ...
  // Check against expected header.
  if (!std::equal(magicHeader, magicHeader + sizeof(magicHeader),
                  bufferGenericHeader)) {
    return false;
  }
  ...
  auto bufferPtr =
      reinterpret_cast<unsigned char const *>(bufferEncryptedHeader);
  ...
  // Use the supplied password and the read salt and iteration count to generate
  // the AES key.
  std::array<uint8_t, 32> key = {0};

  auto keyBytes = pbkdf2->derive_key(key.max_size(), password, salt.data(),
  ...
  inputStream.read(reinterpret_cast<char *>(aeadData.data()),
                   aeadData.capacity());
  ...
  // Read file in chunks and feed it to the decrypter. First, determine how much
  // data must be present in the last block feed to the decrypter.
  ...
  size_t const remainingEncryptedDataLength =
      fileSize - currentPosition - minimumTrailingBlockLength;
  size_t const blockLength = cipher->update_granularity();
  size_t const chunks =
      remainingEncryptedDataLength / cipher->update_granularity();
  size_t const lastChunkLength =
      remainingEncryptedDataLength % cipher->update_granularity() +
      minimumTrailingBlockLength;
