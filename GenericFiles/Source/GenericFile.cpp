...
static constexpr std::array<uint8_t, 12> magicBytes = {
...
GenericFile::GenericFile(boost::filesystem::path const &filePath) {
  ...
  std::ifstream fileSource(filePath.string(),
...
uint32_t GenericFile::getDeviceID() const { return implementation->deviceID; }
...
GenericFile::GenericFileImplementation::GenericFileImplementation(
    std::vector<uint8_t> const &data) {
  // Ensure enough data is present to construct the implementation.
  if (data.size() < headerBytes) {
    throw std::runtime_error("Could not create GenericFileImplementation, "
                             "since too few data bytes were present.");
  } else if (data.size() > headerBytes) {
    throw std::runtime_error("Could not create GenericFileImplementation, "
                             "since too many data bytes were present.");
  }
  ...
  switch (data[14]) {
  case static_cast<std::underlying_type<GenericFileType>::type>(
      GenericFileType::CompressedFile):
    genericFileType = GenericFileType::CompressedFile;
    break;
  case static_cast<std::underlying_type<GenericFileType>::type>(
      GenericFileType::EncryptedFile):
