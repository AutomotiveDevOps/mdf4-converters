...
void ASC_CAN_Exporter::writeRecord(CANRecord const &record) {
  ...
  fmt::print(output,
             ...
             deltaTime.count(), record.BusChannel, record.ID,
             record.IDE ? 'x' : ' ', (record.Dir == 0) ? "Rx" : "Tx",
             record.DLC, fmt::join(record.DataBytes, " "));
}
...
std::string
ASC_CAN_Exporter::convertTimestamp(std::chrono::seconds const &timeStamp) {
...
std::string
ASC_CAN_Exporter::convertTimestamp(std::chrono::nanoseconds const &timeStamp) {
  // Extract time and convert to right zone.
