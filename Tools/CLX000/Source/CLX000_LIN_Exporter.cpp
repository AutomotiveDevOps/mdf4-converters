...
void CLX000_LIN_Exporter::writeHeader() {
  ...
  int const dateLength =
      convertTimestampWithYear(
...
void CLX000_LIN_Exporter::writeRecord(LINRecord const &record) {
  ...
  if (configuration.DataFields_timestamp) {
    ...
    std::time_t correctedTime = tools::shared::convertTimeStamp(
        displayLocalTime, static_cast<time_t>(timeForConversion.count()),
        fileInfo);
    ...
    fmt::print(output, FMT_STRING("{:s}{:s}"),
               convertTimestampToFormat(correctedTime), delimiter);
  }
...
std::string
CLX000_LIN_Exporter::convertTimestampWithYear(std::time_t const &timeStamp) {
  // Reset buffer.
  timeStampStream.str("");
...
std::string
CLX000_LIN_Exporter::convertTimestampToFormat(std::time_t const &timeStamp) {
  // Reset label.
  timeStream.str("");
...
void CLX000_LIN_Exporter::convertTimestampToFormat0(
...
void CLX000_LIN_Exporter::convertTimestampToFormat1(
    std::time_t const &timeStamp) {
  double mainPart, floatingPart;
  floatingPart = std::modf(timeStamp * 1E-9, &mainPart);
...
void CLX000_LIN_Exporter::convertTimestampToFormat2(
    std::time_t const &timeStamp) {
  double mainPart, floatingPart;
  floatingPart = std::modf(timeStamp * 1E-9, &mainPart);
...
void CLX000_LIN_Exporter::convertTimestampToFormat3(
    std::time_t const &timeStamp) {
  double mainPart, floatingPart;
  floatingPart = std::modf(timeStamp * 1E-9, &mainPart);
...
void CLX000_LIN_Exporter::convertTimestampToFormat4(
    std::time_t const &timeStamp) {
  double mainPart, floatingPart;
  floatingPart = std::modf(timeStamp * 1E-9, &mainPart);
...
void CLX000_LIN_Exporter::convertTimestampToFormat5(
    std::time_t const &timeStamp) {
  double mainPart, floatingPart;
  floatingPart = std::modf(timeStamp * 1E-9, &mainPart);
...
void CLX000_LIN_Exporter::convertTimestampToFormat6(
    std::time_t const &timeStamp) {
  double mainPart, floatingPart;
  floatingPart = std::modf(timeStamp * 1E-9, &mainPart);
