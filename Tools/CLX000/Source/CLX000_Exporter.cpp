...
CLX000_Exporter::CLX000_Exporter()
    : ConverterInterface(std::string(PROGRAM_NAME), true), logOptions("log"),
      dataFieldsOptions("dataFields") {
  logOptions.add_options()("log.valueSeparator",
                           bpo::value<int>()->default_value(32), "")(
  ...
  dataFieldsOptions.add_options()("dataFields.timestamp",
...
bool CLX000_Exporter::convert(boost::filesystem::path inputFilePath,
  ...
  if (mdfFile) {
    ...
    if (info.CANMessages > 0) {
      ...
      auto const outputFilePathCAN_1 =
          outputFolder / (outputFileBase + "CAN_1.txt");
      auto const outputFilePathCAN_2 =
          outputFolder / (outputFileBase + "CAN_2.txt");
      ...
      CLX000_CAN_Exporter exporter_1(output_1, info, 1, configuration,
                                     commonOptions->displayTimeFormat);
      CLX000_CAN_Exporter exporter_2(output_2, info, 2, configuration,
                                     commonOptions->displayTimeFormat);
    ...
    if (info.LINMessages > 0) {
      ...
      auto const outputFilePathLIN_1 =
          outputFolder / (outputFileBase + "LIN_1.txt");
      auto const outputFilePathLIN_2 =
          outputFolder / (outputFileBase + "LIN_2.txt");

      // Create exporters for each channel and perform common setup.
      std::ofstream output_1(outputFilePathLIN_1.string());
      std::ofstream output_2(outputFilePathLIN_2.string());
      CLX000_LIN_Exporter exporter_1(output_1, info, 1, configuration,
                                     commonOptions->displayTimeFormat);
      CLX000_LIN_Exporter exporter_2(output_2, info, 2, configuration,
                                     commonOptions->displayTimeFormat);
...
void CLX000_Exporter::configureFileParser(
    boost::program_options::options_description &opts) {
  opts.add(logOptions);
  opts.add(dataFieldsOptions);
}

Version CLX000_Exporter::getVersion() const { return version; }

tools::shared::ParseOptionStatus CLX000_Exporter::parseOptions(
    boost::program_options::variables_map const &result) {
  // Read out exporter options.
  configuration.ValueSeparator[0] =
      static_cast<char>(result["log.valueSeparator"].as<int>());
  configuration.TimestampFormat = result["log.timestampFormat"].as<int>();
  configuration.TimestampTimeSeparator[0] =
      static_cast<char>(result["log.timestampTimeSeparator"].as<int>());
  configuration.TimestampTimeMsSeparator[0] =
      static_cast<char>(result["log.timestampTimeMsSeparator"].as<int>());
  configuration.TimestampDateSeparator[0] =
      static_cast<char>(result["log.timestampDateSeparator"].as<int>());
  configuration.TimeTimeDateSeparator[0] =
      static_cast<char>(result["log.timeTimeDateSeparator"].as<int>());

  configuration.DataFields_timestamp =
      result["dataFields.timestamp"].as<bool>();
  configuration.DataFields_type = result["dataFields.type"].as<bool>();
  configuration.DataFields_id = result["dataFields.id"].as<bool>();
  configuration.DataFields_dataLength =
      result["dataFields.dataLength"].as<bool>();
