...
CLX000_CAN_Exporter::CLX000_CAN_Exporter(
    ...
    : GenericRecordExporter(output), fileInfo(fileInfo), busChannel(busChannel),
...
void CLX000_CAN_Exporter::writeHeader() {
  ...
  output << "# Logger type" << separator
         << static_cast<unsigned int>(fileInfo.LoggerType) << "\n";
  output << "# HW rev" << separator << fileInfo.HW_Version << "\n";
  output << "# FW rev" << separator << fileInfo.FW_Version << "\n";
  output << "# Logger ID" << separator << std::hex << std::setfill('0')
         << std::setw(8) << fileInfo.LoggerID << std::dec << "\n";
  output << "# Session No." << separator
         << static_cast<unsigned int>(fileInfo.Session) << "\n";
  output << "# Split No." << separator
         << static_cast<unsigned int>(fileInfo.Split) << "\n";
  ...
  std::chrono::seconds const fileTime =
      std::chrono::duration_cast<std::chrono::seconds>(fileInfo.Time);
  ...
  output << "# Time format" << separator << configuration.TimestampFormat
         << "\n";
  output << "# Time separator" << separator << "\""
         << std::string_view(configuration.TimestampTimeSeparator) << "\""
         << "\n";
  output << "# Time separator ms" << separator << "\""
         << std::string_view(configuration.TimestampTimeMsSeparator) << "\""
         << "\n";
  output << "# Date separator" << separator << "\""
         << std::string_view(configuration.TimestampDateSeparator) << "\""
         << "\n";
...
void CLX000_CAN_Exporter::writeRecord(CANRecord const &record) {
  if (!timeStampSet) {
    ...
    timeStampSet = true;
    headerTimeStamp =
        std::chrono::duration_cast<std::chrono::seconds>(record.TimeStamp)
            .count();
  }

  if (configuration.DataFields_timestamp) {
    convertTimestampToFormat(record.TimeStamp, output);
    fmt::print(output, FMT_STRING("{:s}"), delimiter);

    /*
    // Convert to right zone.
    std::chrono::nanoseconds logTime(record.TimeStamp);
    std::chrono::seconds timeForConversion =
    std::chrono::duration_cast<std::chrono::seconds>(logTime);
    std::chrono::nanoseconds floatingPart = logTime - timeForConversion;

    std::time_t correctedTime =
    tools::shared::convertTimeStamp(displayLocalTime,
  ...
  if (configuration.DataFields_type) {
    fmt::print(output, FMT_STRING("{:b}{:s}"), record.IDE, delimiter);
  }

  if (configuration.DataFields_id) {
    fmt::print(output, FMT_STRING("{:x}{:s}"), record.ID, delimiter);
  }
  if (configuration.DataFields_dataLength) {
    fmt::print(output, FMT_STRING("{:x}{:s}"), record.DataLength, delimiter);
  }
  if (configuration.DataFields_data) {
    fmt::print(output, FMT_STRING("{:02x}"), fmt::join(record.DataBytes, ""));
  }

  fmt::print(output, "\n");
}

void CLX000_CAN_Exporter::convertTimestampWithYear(std::time_t const &timeStamp,
                                                   std::ostream &output) {
  // Convert to right zone.
  std::time_t correctedTime =
      tools::shared::convertTimeStamp(displayLocalTime, timeStamp, fileInfo);

  // Convert epoch to datetime.
  std::tm const tm = *std::gmtime(&correctedTime);

  fmt::print(output, "{:%Y%m%dT%H%M%S}", tm);
}

void CLX000_CAN_Exporter::convertTimestampToFormat(
    std::chrono::nanoseconds const &timeStamp, std::ostream &output) {
  std::chrono::seconds const secondsComponent =
      std::chrono::duration_cast<std::chrono::seconds>(timeStamp);
  std::chrono::milliseconds const millisComponent =
  ...
  // NOTE: When using std::tm for date-time output, compile time checks on the
  // format string cannot be
  //       performed. See https://github.com/fmtlib/fmt/issues/1269

  // The different timestamp formats are inclusive. Check for the longest
  // edition, and filter down.
  switch (configuration.TimestampFormat) {
  case 6:
    fmt::print(output, "{:%Y}{:s}", genericTimeStamp,
               configuration.TimestampDateSeparator);
    // Fallthrough
  case 5:
    fmt::print(output, "{:%m}{:s}", genericTimeStamp,
               configuration.TimestampDateSeparator);
    // Fallthrough
  case 4:
    fmt::print(output, "{:%d}{:s}", genericTimeStamp,
               configuration.TimeTimeDateSeparator);
    // Fallthrough
  case 3:
    fmt::print(output, "{:%H}{:s}", genericTimeStamp,
  ...
  case 2:
    ...
    // Fallthrough
  case 1:
    fmt::print(output, "{:%S}{:s}", genericTimeStamp,
               configuration.TimestampTimeMsSeparator);
    // Fallthrough
  case 0:
    fmt::print(output, FMT_STRING("{:03d}"), millisComponent.count());
    break;
  default:
    break;
  }
