...
/**
 * The Converter interface allows each conversion tool to customize the
 * commandline parser with additional parameters, as well as use a default ini
...
public:
  ...
  virtual bool convert(boost::filesystem::path inputFilePath,
                       boost::filesystem::path outputFolder) = 0;

  /**
   * Called during the configuration stage of the commandline parser. Allows for
   * injecting additional options.
   * @param opts
   */
  virtual void
  configureParser(boost::program_options::options_description &opts);

  /**
   * Called during the configuration stage of the configuration file parser.
   * Allows for injecting additional options.
   * @param opts
   */
  virtual void
  configureFileParser(boost::program_options::options_description &opts);

  /**
   * Called after all options has been parsed, to allow the converter to respond
   * to injected options.
   * @param result
   * @return
   */
  virtual ParseOptionStatus
...
protected:
  explicit ConverterInterface(std::string programName,
                              bool usesConfigurationFile = false);
