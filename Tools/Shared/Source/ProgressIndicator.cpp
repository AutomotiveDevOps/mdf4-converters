...
ProgressIndicator::ProgressIndicator(std::size_t start,
                                     std::size_t stop,
                                     bool suppressed)
    : start(start), stop(stop), suppressed(suppressed) {
...
void ProgressIndicator::update_percent(std::size_t current) {
  ...
  double ratio = static_cast<double>(current - start) * 1.0 /
  ...
  fmt::print(rightStream, FMT_STRING("] ( {:{}} / {:} )"), current,
             numDigits(stop), stop);

  std::string endString = rightStream.str();
  stringLength += endString.size();

  std::size_t const filledSpaces =
      std::round(ratio * static_cast<double>(width - stringLength));
...
void ProgressIndicator::setPrefix(std::string value) { prefix = value; }

void ProgressIndicator::setSuffix(std::string value) { suffix = value; }
