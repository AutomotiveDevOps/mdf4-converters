...
static std::vector<std::string> acceptableExtensions = {".mf4", ".mfe", ".mfc",
                                                        ".mfm"};
...
StatusCode ExecutableInterface::main(int argc, char **argv) {
  ...
  // Parse options. Again, start with basic and continue to derived. The
  // commandline parser is allowed to take ALL options, to ensure a default
  // value is populated.
  ...
  try {
    ...
    case bpo::invalid_command_line_syntax::missing_parameter:
      BOOST_LOG_TRIVIAL(error)
          << "Missing argument for option '" << e.get_option_name() << "'";
      std::cout << "Missing argument for option '" << e.get_option_name()
                << "'";
      return StatusCode::MissingArgument;
    default:
      BOOST_LOG_TRIVIAL(fatal)
          << "Could not complete parsing due to exception: " << e.what();
      return StatusCode::CriticalError;
    }
  } catch (std::exception &e) {
    BOOST_LOG_TRIVIAL(fatal)
        << "Error occurred during initial input argument parsing: " << e.what();
    return StatusCode::CriticalError;
  }
  ...
  if (interface->usesConfigFile()) {
    ...
    earlyLogMessages.emplace_back("Converter has requested config file data, "
                                  "attempting to load relative to executable");
    bfs::path fullPath = boost::dll::program_location();
    auto configFilePath = fullPath.parent_path() /
                          (std::string(interface->programName) + "_config.ini");

    ss << "Attempting to load additional settings from config file at: "
       << configFilePath;
    ...
    if (bfs::exists(configFilePath)) {
      try {
        ...
        auto configParserOptions =
            bpo::parse_config_file(configFile, configFileOptions, true);
      ...
      ss << "Error during parsing of configuration file. Could not locate file "
            "at: "
         << configFilePath;
      earlyLogMessages.emplace_back(ss.str());
      ss.str("");
  ...
  // If no arguments are supplied, or an error occurred during parsing, display
  // the help message.
  if (argc <= 1) {
  ...
  try {
    ...
    BOOST_LOG_TRIVIAL(fatal)
        << "Error occurred during general input argument parsing: " << e.what();
  ...
  try {
    ...
    BOOST_LOG_TRIVIAL(fatal)
        << "Error occurred during specialized input argument parsing: "
        << e.what();
    return StatusCode::InputArgumentParsingError;
  } catch (std::exception &e) {
    BOOST_LOG_TRIVIAL(fatal)
  ...
  std::vector<std::string> unrecognizedOptions =
      bpo::collect_unrecognized(parsedOptions.options, bpo::exclude_positional);
  ...
  if (noConfigFileFound) {
    bool failOnMissingConfigFile =
        (optionResult.count("error-on-missing-config-file") != 0);

    if (failOnMissingConfigFile) {
      BOOST_LOG_TRIVIAL(fatal)
          << "No configuration file found, but required by the user, exiting.";
      return StatusCode::ConfigurationFileParseError;
    } else {
  ...
  if ((status & ParseOptionStatus::UnrecognizedOption) ==
      ParseOptionStatus::UnrecognizedOption) {
    displayUnrecognizedOptions(unrecognizedOptions);
    return StatusCode::UnrecognizedOption;
  } else if ((status & ParseOptionStatus::DisplayHelp) ==
             ParseOptionStatus::DisplayHelp) {
    displayHelp();
    return StatusCode::NoErrors;
  } else if ((status & ParseOptionStatus::DisplayVersion) ==
             ParseOptionStatus::DisplayVersion) {
    displayVersion();
    return StatusCode::NoErrors;
    ...
    return StatusCode::InvalidInputArgument;
  } else if ((status & ParseOptionStatus::NoInputFiles) ==
             ParseOptionStatus::NoInputFiles) {
    return StatusCode::NoErrors;
  }

  std::vector<boost::filesystem::path> curatedInputFiles;

  // Create a list of files to work on. For each folder in the input, walk it
  ...
  for (auto &path : inputFiles) {
    ...
    if (bfs::is_directory(inputPath)) {
      BOOST_LOG_TRIVIAL(info)
          << "Received folder as input argument, performing recursive walk: "
          << inputPath;
      for (auto &entry : bfs::recursive_directory_iterator(
               inputPath, bfs::symlink_option::no_recurse)) {
        std::string extension = entry.path().extension().string();
        boost::algorithm::to_lower(extension);

        if (std::find(std::cbegin(acceptableExtensions),
                      std::cend(acceptableExtensions),
                      extension) != std::end(acceptableExtensions)) {
          BOOST_LOG_TRIVIAL(info)
              << "Found file with matching extensions; " << entry;
  ...
  // Create a mapping between all input files and their corresponding output
  // files. If the output directory is set, override the destination path, else
  // place them in the same folder as the input file.
  ...
  for (auto &path : curatedInputFiles) {
    ...
    if (optionResult.count("output-directory")) {
      ...
      outputFolder = boost::filesystem::path(
          optionResult["output-directory"].as<std::string>());
      ...
      if (!boost::filesystem::exists(outputFolder)) {
        BOOST_LOG_TRIVIAL(info)
            << "Output folder does not exists. Creating " << outputFolder << "";
        try {
          boost::filesystem::create_directories(outputFolder);
        } catch (boost::filesystem::filesystem_error &e) {
          BOOST_LOG_TRIVIAL(fatal) << "Could not create output folder "
                                   << outputFolder << ". Logged error is:\n"
                                   << e.what();
          return StatusCode::CriticalError;
        }
      }
    ...
    if (mdf::generic::getGenericFileType(inputFilePath) ==
        mdf::generic::GenericFileType::EncryptedFile) {
      ...
      if (!passwordStorage) {
        auto localPasswordFilePath =
            inputFilePath.parent_path() / "passwords.json";

        if (boost::filesystem::exists(localPasswordFilePath)) {
          try {
            currentPasswordStorage = std::make_shared<PasswordStorage>(
                localPasswordFilePath.string());
          } catch (std::exception &e) {
            BOOST_LOG_TRIVIAL(error)
                << "Could not load local password file, even though the file "
                   "exists. Input file: "
                << inputFilePath << ".";
            continue;
          }
          ...
          BOOST_LOG_TRIVIAL(error)
              << "Could not decrypt file, as no password file is specified, no "
                 "default password file is found and no local password file is "
                 "found. Input file: "
              << inputFilePath << ".";
      ...
      bfs::path temporaryDirectoryPath =
          bfs::temp_directory_path() / bfs::unique_path();
      BOOST_LOG_TRIVIAL(info)
          << "Creating temporary folder: " << temporaryDirectoryPath << ".";
      bfs::create_directories(temporaryDirectoryPath);
      bfs::path temporaryPath =
          temporaryDirectoryPath / inputFilePath.filename();

      // Attempt to use the correct naming scheme.
      if (boost::algorithm::iequals(temporaryPath.extension().string(),
                                    ".MFM")) {
        temporaryPath.replace_extension(".MFC");
      } else if (boost::algorithm::iequals(temporaryPath.extension().string(),
                                           ".MFE")) {
      ...
      if (passwordAvailable != PasswordType::Missing) {
        std::string const &password =
            currentPasswordStorage->getPassword(device);

        // Decrypt to temporary file, and pass this on for further processing.
        decryptionStatus = mdf::decryptFile(inputFilePath.string(),
                                            temporaryPath.string(), password);
      }

      // Check if the current file is a temporary one. If so, then delete it.
      if (inputFileIsTemporary) {
        BOOST_LOG_TRIVIAL(info)
            << "Deleting temporary file: " << inputFilePath << ".";
      ...
      if (!decryptionStatus) {
        std::stringstream ss;
        ss << "Error during conversion of " << inputFilePath
           << ". Failed to perform decryption - ";
        ...
        case PasswordType::Missing:
          // Fallthrough.
        default:
          ss << "no default password and/or no matching serial number found in "
                "password file.";
          break;
        }

        ss << "Attempted to use "
           << currentPasswordStorage->getPasswordFilePath()
           << " as password list.";
    ...
    if (mdf::generic::getGenericFileType(inputFilePath) ==
        mdf::generic::GenericFileType::CompressedFile) {
      ...
      if (boost::algorithm::equals(temporaryPath.extension().string(),
                                   ".MFC")) {
        temporaryPath.replace_extension(".MF4");
      }

      // Decompress to temporary file, and pass this on for further processing.
      bool decompressionStatus =
          mdf::decompressFile(inputFilePath.string(), temporaryPath.string());

      // Check if the current file is a temporary one. If so, then delete it.
      if (inputFileIsTemporary) {
        BOOST_LOG_TRIVIAL(info)
            << "Deleting temporary file and parent folder: " << inputFilePath
            << ".";
        bfs::remove(inputFilePath);
        bfs::remove(inputFilePath.parent_path());
      }
      ...
      if (!decompressionStatus) {
        BOOST_LOG_TRIVIAL(error)
            << "Error during conversion of " << inputFilePath
...
void ExecutableInterface::configureParser() {
  // Add default options.
  commandlineOptions.add_options()("help,h",
                                   bpo::bool_switch()->default_value(false),
                                   "Print this help message.")(
      "version,v", bpo::bool_switch()->default_value(false),
      "Print version information.")("verbose",
                                    bpo::value<int>()->default_value(1),
                                    "Set verbosity of output (0-5).")(
      "input-directory,I", bpo::value<std::string>(),
      "Input directory to convert files from.")(
      "output-directory,O", bpo::value<std::string>(),
      "Output directory to place converted files into.")(
      ...
      "Display times in UTC (u), logger localtime (l, default) or PC local "
      "time (p).")(
      "password-file,p", bpo::value<std::string>(),
      "Path to password json file. If left empty, and an encrypted file is "
      "encountered, the folder of the input file will be searched.")(
      "input-files,i", bpo::value<std::vector<std::string>>(),
      "List of files to convert, ignored if input-directory is specified. All "
      "unknown arguments will be interpreted as input files.");
...
void ExecutableInterface::displayHelp() const {
  std::cout << "Usage:"
            << "\n";
  std::cout << interface->programName
            << " [-short-option value --long-option value] [-i] file_a [file_b "
               "...]:\n";
  std::cout << "\n";
  std::cout << "Short options start with a single \"-\", while long options "
               "start with \"--\".\n";
  std::cout << "A value enclosed in \"[]\" signifies it is optional.\n";
  std::cout << "Some options only exists in the long form, while others exist "
               "in both forms.\n";
...
void ExecutableInterface::displayUnrecognizedOptions(
...
void ExecutableInterface::displayVersion() const {
  // Display version information on the base library, the shared base and the
  // specialization.
  std::cout << "Version of " << interface->programName << ": "
            << interface->getVersion() << std::endl;
  std::cout << "Version of converter base: " << mdf::tools::shared::version
            << std::endl;
  std::cout << "Version of MDF library: " << mdf::version << std::endl;
}

ParseOptionStatus ExecutableInterface::parseOptions(
    boost::program_options::variables_map const &result) {
  ...
  case 1:
    ...
    break;
  case 2:
    blo::core::get()->set_filter(blo::trivial::severity >=
                                 blo::trivial::warning);
    break;
  case 3:
  ...
  default:
    // Unknown value.
    std::string const originalToken =
        fmt::format(FMT_STRING("{:d}"), verboseToken);

    throw bpo::validation_error(
        bpo::validation_error::kind_t::invalid_option_value, "verbose",
        originalToken);
  }
  ...
  if (boost::algorithm::iequals("u", timeZoneDisplay)) {
    ...
    // Unknown value.
    throw bpo::validation_error(
        bpo::validation_error::kind_t::invalid_option_value, "timezone",
  ...
  if (result.count("password-file")) {
    ...
    boost::filesystem::path passwordFilePath =
        boost::filesystem::weakly_canonical(passwordFile);

    BOOST_LOG_TRIVIAL(trace)
        << "Attempting to load password file from argument at "
        << passwordFilePath;

    if (boost::filesystem::exists(passwordFilePath)) {
      try {
        passwordStorage =
            std::make_shared<PasswordStorage>(passwordFilePath.string());
      } catch (boost::property_tree::json_parser_error &e) {
        BOOST_LOG_TRIVIAL(fatal)
            << "Error during parsing of " << passwordFilePath << ": "
            << e.message() << " at line " << e.line();
    ...
    boost::filesystem::path passwordFilePath =
        current_path.parent_path() / "passwords.json";

    BOOST_LOG_TRIVIAL(trace)
        << "Attempting to load default password file next to executable at "
        << passwordFilePath;
    ...
    if (boost::filesystem::exists(passwordFilePath)) {
      try {
        ...
        // Ignore password file errors in the default path. User may not need
        // passwords.
      }
    }
  }

  // Is an input directory specified? In that case, ignore any files passed to
  // the program and instead populate the files list from the directory.
  if (result.count("input-directory")) {
    std::string inputDirectoryToken =
        result["input-directory"].as<std::string>();
    ...
    if (!boost::filesystem::exists(inputDirectory)) {
      ...
      for (auto &entry : boost::make_iterator_range(
               boost::filesystem::directory_iterator(inputDirectory), {})) {
    ...
    std::vector<std::string> files =
        result["input-files"].as<std::vector<std::string>>();
