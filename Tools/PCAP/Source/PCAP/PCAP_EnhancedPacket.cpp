...
void PCAP_EnhancedPacket::serialize(std::ostream &stream) const {
  stream.write(reinterpret_cast<char const *>(&interfaceIndex),
               sizeof(interfaceIndex));
  stream.write(reinterpret_cast<char const *>(&timeStampHigh),
               sizeof(timeStampHigh));
  stream.write(reinterpret_cast<char const *>(&timeStampLow),
               sizeof(timeStampLow));
  stream.write(reinterpret_cast<char const *>(&capturedPacketLength),
               sizeof(capturedPacketLength));
  stream.write(reinterpret_cast<char const *>(&originalPacketLength),
               sizeof(originalPacketLength));
