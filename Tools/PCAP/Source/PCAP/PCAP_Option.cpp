...
PCAP_Option::PCAP_Option(uint16_t optionCode)
    : optionCode(optionCode), optionLength(0) {}
...
uint16_t PCAP_Option::getOptionCode() const { return optionCode; }

std::ostream &operator<<(std::ostream &stream, PCAP_Option const &data) {
  uint16_t optionLength = data.getSize() - 4;

  // Write the header data.
  stream.write(reinterpret_cast<char const *>(&data.optionCode),
               sizeof(data.optionCode));
  stream.write(reinterpret_cast<char const *>(&optionLength),
               sizeof(optionLength));
