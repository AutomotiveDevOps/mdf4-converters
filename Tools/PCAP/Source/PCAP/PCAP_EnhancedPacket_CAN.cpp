...
PCAP_EnhancedPacket_CAN::PCAP_EnhancedPacket_CAN(
    mdf::CANRecord const &canDataFrame)
    : data({0}) {
  ...
  // Update the timestamps. Since no 'if_tsresol' has been set, the standard
  // resolution of microseconds is assumed.
  std::chrono::nanoseconds rawTimeStamp(canDataFrame.TimeStamp);
  std::chrono::microseconds timeStampMicros =
      std::chrono::duration_cast<std::chrono::microseconds>(rawTimeStamp);
