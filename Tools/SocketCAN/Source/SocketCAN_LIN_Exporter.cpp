...
SocketCAN_LIN_Exporter::SocketCAN_LIN_Exporter(
    std::ostream &output,
    FileInfo const &fileInfo,
    tools::shared::DisplayTimeFormat displayLocalTime)
    : GenericRecordExporter(output), fileInfo(fileInfo),
...
void SocketCAN_LIN_Exporter::writeRecord(LINRecord const &record) {
  ...
  // * ID is either 3 hex characters or 8 hex characters, depending on the
  // status of the extended ID. Fill with 0.
  // * F is flags in the FD case, not used in the classic case.
  // * XX corresponds to data in hex. The length of this must match the DLC,
  // such that a DLC of 2 corresponds to
  //   4 characters etc.

  // Convert to right zone.
  std::time_t correctedTime = tools::shared::convertTimeStamp(
      displayLocalTime, record.TimeStamp.count(), fileInfo);
