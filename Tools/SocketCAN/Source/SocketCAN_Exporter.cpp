...
SocketCAN_Exporter::SocketCAN_Exporter()
    : ConverterInterface("mdf2socketcan") {}

bool SocketCAN_Exporter::convert(boost::filesystem::path inputFilePath,
  ...
  if (mdfFile) {
    ...
    if (info.CANMessages > 0) {
      ...
      SocketCAN_CAN_Exporter exporter(output, info,
    ...
    if (info.LINMessages > 0) {
      ...
      SocketCAN_LIN_Exporter exporter(output, info,
                                      commonOptions->displayTimeFormat);
