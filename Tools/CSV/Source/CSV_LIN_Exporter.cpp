...
void CSV_LIN_Exporter::writeRecord(LINRecord const &record) {
  ...
  fmt::print(output,
             ...
             record.BusChannel, delimiter, record.ID, delimiter,
             record.DataLength, delimiter, record.Dir, delimiter,
             fmt::join(record.DataBytes.cbegin(),
                       record.DataBytes.cbegin() + record.DataLength, ""));
}
