...
CSV_CAN_Exporter::CSV_CAN_Exporter(std::ostream &output, std::string delimiter)
    : GenericRecordExporter(output), delimiter(std::move(delimiter)) {
  //
}
...
void CSV_CAN_Exporter::writeRecord(mdf::CANRecord const &record) {
  // TODO: Switch timestamp to locale aware solution once libfmt implements
  // ":Ln".
  ...
  fmt::print(output,
             ...
             record.BusChannel, delimiter, record.ID, delimiter, record.IDE,
             delimiter, record.DLC, delimiter, record.DataLength, delimiter,
             record.Dir, delimiter, record.EDL, delimiter, record.BRS,
             delimiter, fmt::join(record.DataBytes, ""));
}
