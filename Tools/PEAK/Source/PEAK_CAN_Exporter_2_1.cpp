...
PEAK_CAN_Exporter_2_1::PEAK_CAN_Exporter_2_1(std::ostream &output,
                                             FileInfo const &fileInfo)
    : PEAK_CAN_Exporter(output, fileInfo) {}
...
void PEAK_CAN_Exporter_2_1::correctHeader() {
  if (timeStampSet) {
    ...
    // Convert the time to days since 30th December, 1899. Utilize that there
    // are 25568.0 days between this and epoch start time.
    constexpr long secondsInADay = 24 * 60 * 60;
    using daysEpoch =
        std::chrono::duration<double, std::ratio<secondsInADay, 1>>;

    // Days since 1970.
    daysEpoch daysSinceEpoch =
        std::chrono::duration_cast<daysEpoch>(headerTimeStamp);
...
void PEAK_CAN_Exporter_2_1::writeHeader() {
  ...
  // Write a placeholder for the start time, since this is not known until the
...
void PEAK_CAN_Exporter_2_1::writeRecord(CANRecord const &record) {
  ...
  if (record.IDE) {
    fmt::print(
        ...
        recordCounter++, timeStamp.count(), messageType, record.BusChannel,
        record.ID, (record.Dir == 0) ? "Rx" : "Tx", record.DLC,
