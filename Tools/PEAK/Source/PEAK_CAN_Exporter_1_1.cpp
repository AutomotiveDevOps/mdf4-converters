...
PEAK_CAN_Exporter_1_1::PEAK_CAN_Exporter_1_1(std::ostream &output,
...
void PEAK_CAN_Exporter_1_1::correctHeader() {
  if (timeStampSet) {
    ...
    output << std::setw(10) << std::setfill('0')
           << static_cast<long long>(fractional * 1E10) << " ";
    output << std::setfill(' ');
  }
}
...
void PEAK_CAN_Exporter_1_1::writeRecord(CANRecord const &record) {
  ...
  if (record.IDE) {
    fmt::print(
        output, FMT_STRING("{:6d}) {:11.1f} {:s}    {:08X} {:d} {:02X}\n"),
        recordCounter++, timeStamp.count(), (record.Dir == 0) ? "Rx" : "Tx",
        record.ID, record.DLC, fmt::join(record.DataBytes, " "));
  } else {
    fmt::print(
        output, FMT_STRING("{:6d}) {:11.1f} {:s}        {:04X} {:d} {:02X}\n"),
        recordCounter++, timeStamp.count(), (record.Dir == 0) ? "Rx" : "Tx",
        record.ID, record.DLC, fmt::join(record.DataBytes, " "));
  }
